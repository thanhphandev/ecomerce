...
export const generateRefreshToken = async (user: IUser) => {
    ...
    const refresh_token = jwt.sign({ id: user._id, username: user.username, isAdmin: user.isAdmin, isBanned: user.isBanned }, process.env.SECRET_REFRESH_KEY || 'hello', { expiresIn: '1h' });
